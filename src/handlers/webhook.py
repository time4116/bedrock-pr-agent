...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            "Received webhook event", {"path": event.get("path"), "method": event.get("httpMethod")}
        )
        ...
        if not is_repo_allowed(repo_full_name):
            ...
            return {
                "statusCode": 200,
                "body": json.dumps({"message": "Repository not in allowed list"}),
            }

        if not _check_rate_limit(repo_full_name):
            logger.info(
                "Rate limit reached for repo", {"repo": repo_full_name, "limit": _WEEKLY_LIMIT}
            )
            return {
                "statusCode": 200,
                "body": json.dumps({"message": "Weekly review limit reached"}),
            }
        ...
        logger.info(
            ...
            {"repo": repo_full_name, "pr_number": pr_number, "action": action},
        )
